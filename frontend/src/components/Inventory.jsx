...
export default function Inventory() {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/products${isSuperAdmin ? `?shop_id=${selectedShopId}` : ''}`, {
        method: 'POST',
        headers: {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/products/${currentProduct.id}${isSuperAdmin ? `?shop_id=${selectedShopId}` : ''}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/products/${productId}${isSuperAdmin ? `?shop_id=${selectedShopId}` : ''}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const resData = await response.json();
  ...
  const handleCsvUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/products/bulk-upload${isSuperAdmin ? `?shop_id=${selectedShopId}` : ''}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
